...
```
composer require lchhieu/cassandra
```
...
```php
lchhieu\Cassandra\CassandraServiceProvider::class,
```
...
```php
$app->register('lchhieu\Cassandra\CassandraServiceProvider');
...
```php
use lchhieu\Cassandra\Eloquent\Model as Eloquent;
...
```php
'Moloquent'       => 'lchhieu\Cassandra\Eloquent\Model',
```
...
```php
'lchhieu\Cassandra\Auth\PasswordResetServiceProvider',
```
...
```php
use lchhieu\Cassandra\Eloquent\SoftDeletes;
...
*There is also support for upsert operations, check https://github.com/lchhieu/laravel-cassandra#cassandra-specific-operations*
...
If you're using a hybrid Cassandra and SQL setup, you're in luck! The model will automatically return a Cassandra- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-models will need use the `lchhieu\Cassandra\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.

Example SQL-based User model:

```php
use lchhieu\Cassandra\Eloquent\HybridRelations;
